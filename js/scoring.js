 ...
 * 四项各 0–10，总分 = 学习×35% + 赛考×35% + 答疑×20% + 画像×10%
 */
...
export function scoreAssessment(input) {
  ...
  const learning = attachExplain(
    scoreLearning(script, student),
    explainLearning(script, student)
  );
  const competition = attachExplain(
    scoreCompetition(script, student),
    explainCompetition(script, student)
  );
  const qna = attachExplain(
  ...
  const { learnScore, compScore, qnaScore } = applyCourseDeductions(
    learning,
    competition,
    qna,
    courseDeductions
